                        ...
                        foreach ($holooProducts->result as $HolooProd) {
                            if ($WCProd->meta_data[0]->value == $HolooProd->a_Code) {
                                ...
                                if (
                                    ...
                                    return $this->sendResponse('همه محصولات به روز رسانی شدند.', Response::HTTP_OK, (isset($config->update_product_price) && $config->update_product_price=="1") && ($WCProd->regular_price != $this->get_price_type($config->sales_price_field,$HolooProd)) ? $this->get_price_type($config->sales_price_field,$HolooProd) : 0 );
